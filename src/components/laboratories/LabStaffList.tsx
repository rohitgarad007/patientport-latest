...
import { Search, Plus, Mail, Phone, MapPin, MoreHorizontal, Edit, Trash2, Building, User } from "lucide-react";
import Swal from "sweetalert2";
import AddLabStaffForm from "@/components/laboratories/AddLabStaffForm";
import { fetchLaboratoryStaff, addLaboratoryStaff, updateLaboratoryStaff, deleteLaboratoryStaff, changeLaboratoryStaffStatus, fetchActiveLaboratories } from "@/services/masterLaboratoryService";
...
export default function LabStaffList() {
  ...
  const loadStaff = async () => {
    try {
      ...
      const mapped = (res?.data || []).map((s: any) => ({
        ...s,
        status: s.status === "1" ? "active" : s.status, // Ensure status is consistent
      }));
      setStaff(mapped);
  ...
  const loadActiveLabs = async () => {
    try {
      const res = await fetchActiveLaboratories();
      if (res.success) {
        setActiveLabs(res.data);
      }
    } catch (e) {
      console.error(e);
    }
  ...
  useEffect(() => { loadStaff(); loadActiveLabs(); }, [page, searchTerm]);
  ...
  const handleStatus = async (id: string, current: "active" | "inactive") => {
    try {
      const res = await changeLaboratoryStaffStatus(id, current === "active" ? "inactive" : "active");
      if (res.success) {
        loadStaff();
      } else {
          ...
          <div className="flex items-center justify-between">
            ...
            <Dialog>
              ...
              <DialogContent className="sm:max-w-[600px]">
                ...
                <AddLabStaffForm onSubmit={handleAdd} activeLaboratories={activeLabs} />
              </DialogContent>
            </Dialog>
          </div>
        </CardHeader>
                    ...
                    <TableCell className="text-right">
                      <DropdownMenu>
                        ...
                        <DropdownMenuContent align="end">
                          ...
                          <DropdownMenuItem onClick={() => handleStatus(s.staff_uid, s.status)}>

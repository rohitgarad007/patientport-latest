...
export default function StaffFormDialog({ open, onOpenChange, onSubmit, initialData }: StaffFormDialogProps) {
  ...
  const validateField = (name: string, value: any) => {
    switch (name) {
      ...
      case "department":
        ...
        return "";
      case "hospital":
        if (!value) return "Please select hospital";
        return "";
      case "shift":
  ...
  const isFormValid = () => Object.keys(formErrors).length === 0 && 
    formData.name && formData.phone && formData.role && formData.department && formData.hospital &&
  ...
  const handleSubmit = async () => {
    // Validate all fields
    const fields = ["name","phone","role","department","hospital","shift","experienceYears","experienceMonths","captcha"];
  ...
  useEffect(() => {
    if (open) {
      ...
      fetchHospitals().then((res) => { if (res.success) setHospitals(res.data.map((h: any) => ({ label: h.name, value: h.hosuid }))); });
    }
  }, [open]);
        ...
        <div className="space-y-4">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            ...
            <div>
              ...
              <Select options={hospitals} value={formData.hospital} onChange={(val) => handleChange("hospital", val)} placeholder="Select hospital" isSearchable />
              {formErrors.hospital && <p className="text-red-600 text-sm mt-1">{formErrors.hospital}</p>}
            </div>
          </div>

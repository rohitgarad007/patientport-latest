...
export default function HospitalForm({ onSubmit, onCancel, initialData }: HospitalFormProps) {
  ...
  useEffect(() => {
    if (initialData) {
      setFormData({
        ...
        doctorsCount: initialData.doctorsCount || 1,
        staffCount: initialData.staffCount || 1,
      });
    }
  }, [initialData]);
  ...
  return (
    <div className="space-y-4">
      ...
      {/* Doctors Insert + Staff Insert */}
      <div className="grid grid-cols-1 md:grid-cols-4 gap-4">
        <div>
          <Label htmlFor="doctorsCount">Allow Doctors </Label>
          <select
            id="doctorsCount"
            value={formData.doctorsCount}
            onChange={(e) =>
              setFormData({ ...formData, doctorsCount: parseInt(e.target.value) })
            }
        ...
        <div>
          <Label htmlFor="staffCount">Allow Staff</Label>
          <select
            id="staffCount"
            value={formData.staffCount}
            onChange={(e) =>
              setFormData({ ...formData, staffCount: parseInt(e.target.value) })
            }
            className="w-full border rounded-md p-2"
          >
            {Array.from({ length: 100 }, (_, i) => i + 1).map((num) => (

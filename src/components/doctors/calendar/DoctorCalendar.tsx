      ...
      <main className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-6 pt-1">
        <div className="min-h-screen bg-background p-6">
          <motion.div
            ...
            className="mb-8"
          >
            <h1 className="text-2xl font-bold text-foreground pb-4">Manage Calendar</h1>
          </motion.div>
          ...
          <Card className="shadow-card mt-6">
            <CardHeader>
              <CardTitle>Patients • {selectedDate}
                {selectedSlot && (
                  <span
                    className="ml-2 inline-block text-xs px-2 py-1 rounded-md align-middle"
            ...
            <CardContent>
              <Tabs defaultValue="booked" className="space-y-4">
                <TabsList className="grid w-full grid-cols-3 h-auto gap-2 p-2 md:grid-cols-5">
                  <TabsTrigger value="booked" className="text-xs py-3">Booked ({filteredBySlot(grouped.booked).length})</TabsTrigger>
                  <TabsTrigger value="arrived" className="text-xs py-3">Arrived ({filteredBySlot(grouped.arrived).length})</TabsTrigger>
                  <TabsTrigger value="waiting" className="text-xs py-3">Waiting ({filteredBySlot(grouped.waiting).length})</TabsTrigger>
                  <TabsTrigger value="consultation" className="text-xs py-3">Active ({filteredBySlot(grouped.active).length})</TabsTrigger>
                  <TabsTrigger value="completed" className="text-xs py-3">Completed ({filteredBySlot(grouped.completed).length})</TabsTrigger>
                </TabsList>
                  ...
                  {filteredBySlot(grouped.booked).length === 0 ? (
                    ...
                    filteredBySlot(grouped.booked).map((apt) => (
                      <Card key={apt.id} className="p-4">
                        ...
                        <Button size="sm" className="w-full bg-yellow-500 text-black hover:bg-yellow-600" onClick={() => handleMarkArrived(apt.id)}>
                  ...
                  {filteredBySlot(grouped.arrived).length === 0 ? (
                    ...
                    filteredBySlot(grouped.arrived).map((apt) => (
                      <Card key={apt.id} className="p-4">
                        ...
                        <Button size="sm" className="w-full bg-cyan-600 text-white hover:bg-cyan-700" onClick={() => handleMarkToWaiting(apt.id)}>
                  ...
                  {filteredBySlot(grouped.waiting).length === 0 ? (
                    ...
                    filteredBySlot(grouped.waiting).sort((a, b) => (a.queuePosition ?? 0) - (b.queuePosition ?? 0)).map((apt, index, arr) => (
                      <Card key={apt.id} className="p-4 relative">
                        <div className="absolute -top-2 -left-2 w-8 h-8 rounded-full bg-primary text-primary-foreground flex items-center justify-center font-bold text-sm shadow-lg">{index + 1}</div>
                        ...
                        <div className="grid grid-cols-1 gap-2 mb-2">
                          <Button size="sm" className="bg-cyan-600 text-white hover:bg-cyan-700" onClick={() => handleStartConsultation(apt.id)}>
                            <Play className="w-4 h-4 mr-1" /> Start
                          </Button>
                        </div>
                        <div className="flex gap-2">
                          <Button size="sm" variant="outline" className="flex-1" disabled={index === 0} onClick={() => handleReorder(apt.id, 'up')}>
                            <ArrowUp className="w-4 h-4 mr-1" /> Move Up
                          </Button>
                          <Button size="sm" variant="outline" className="flex-1" disabled={index === arr.length - 1} onClick={() => handleReorder(apt.id, 'down')}>
                  ...
                  {filteredBySlot(grouped.active).length === 0 ? (
                    ...
                    filteredBySlot(grouped.active).map((apt) => (
                      <Card key={apt.id} className="p-4 border-status-consultation">
                        ...
                        <Button size="sm" className="w-full bg-green-600 text-white hover:bg-green-700" onClick={() => handleComplete(apt.id)}>
              ...
              <div className="space-y-3">
                {upcoming.slice(0, 8).map((appointment, index) => (
                  <motion.div
                    ...
                    className="p-4 rounded-xl border-2 border-border bg-card hover:shadow-md transition-all"
                  >
                    <div className="flex items-center justify-between">
                        ...
                        <div className="flex-1">
                          ...
                          <div className="flex items-center gap-4 text-sm text-muted-foreground">
                            <div className="flex items-center gap-1">
                              ...
                              <span>{appointment.date} • {fmtTime(appointment.timeSlot?.startTime)} - {fmtTime(appointment.timeSlot?.endTime)}</span>
                            </div>
                            <span>•</span>
                          ...
                          {/* Event type pill (colored like WeeklyCalendar) */}

...
export default function HSMedicalDeclinedRequests() {
  ...
  const loadRequests = async () => {
    try {
      setLoading(true);
      setError("");
      const resp = await fetchMedicalRequests(pagination.page, pagination.limit, "", "Declined");
      setRequests(resp.items);
    } catch (e: any) {
      setError(e?.message || "Failed to load declined requests");
    } finally {
      setLoading(false);
    }
  };
      ...
      <main className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-6 pt-1">
        ...
        <Card className="p-6">
          <div className="space-y-3">
            {loading && <div className="text-sm text-muted-foreground">Loading declined requests...</div>}

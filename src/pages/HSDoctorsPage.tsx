...
import HSDoctorFormDialog from "@/components/doctors/HSDoctorFormDialog";
...
export default function HSDoctorsPage() {
  ...
  const [allowDoctorCount, setAllowDoctorCount] = useState("");
  ...
  useEffect(() => {
    // Load API base URL for building image src paths
    (async () => {
      try {
        const u = await configService.getApiUrl();
        setApiUrl(u);
      } catch (e) {
  ...
  const loadHospitalsFromCookie = () => {
    try {
      ...
      if (cookieData) {
        ...
        setAllowDoctorCount(parsed.doctorsCount);
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="flex items-center justify-between">
        ...
        {doctors.length < Number(allowDoctorCount) ? (
          <Button
            className="flex items-center gap-2"
            onClick={() => {
              setEditingDoctor(null);
                ...
                <TableRow key={doctor.id}>
                  <TableCell>
                    <div className="flex items-center gap-3">
                      <Avatar>
                        {/* Show image if available; otherwise fallback initials */}
